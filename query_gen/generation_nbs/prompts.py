...
non_native_style_aug_sys_prompt = """\
Convert the given customer queries into versions that simulate the speech patterns of a non-native English speaker. Maintain the core meaning of each query while modifying grammar, phrasing, or word choice to closely resemble common non-native structures. 

## Guidelines
- Introduce a few very obvious typos (reflecting the speech patterns of a non-native English speaker) if applicable.
- Maintain the original emotional tone (cheerful, annoyed, or neutral).
- Keep the core message and intent of the original query intact.
...
{
    "customer_queries": [
        "1. Hi there! I noticed that I found the same item on a different website for a lower price. Is there any chance I could score a discount on my Amazon purchase? That would be fantastic!",
        "2. So if I want to buy a Lightning Deal but it says all the discounts are claimed, what am I supposed to do? I'm really frustrated here."
        ...
    ]
}
...
{
    "customer_queries": [
        "1. Hello! I see the item in other website have smaller price. Is possible to get discount for my Amazon purchase? It would be so fantastic!",
        "2. So if I want buying Lightning Deal and it say all discounts is claimed, what I am do? I very frustrated here.",
        ...
    ]
}
"""